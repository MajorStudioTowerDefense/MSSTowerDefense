...
public class TimeBarUI : MonoBehaviour
{
    ...
    IEnumerator shrink()
    {
        ...
        if(GameManager.instance.isTimer)timeBarRect.sizeDelta = new Vector2((timeBarRect.sizeDelta.x-shrinkUnit), timeBarRect.sizeDelta.y);
        timeBarRect = GetComponent<RectTransform>();
    ...
    IEnumerator grow()
    {
        timeBarRect = GetComponent<RectTransform>();
        yield return new WaitForSeconds(0.04f);
        if(GameManager.instance.isTimer)timeBarRect.sizeDelta = new Vector2((timeBarRect.sizeDelta.x + growUnit), timeBarRect.sizeDelta.y);
